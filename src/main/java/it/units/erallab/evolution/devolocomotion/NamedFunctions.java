...
import it.units.erallab.evolution.devolocomotion.Starter.DevoValidationOutcome;
  ...
  public static Accumulator.Factory<Event<?, ? extends UnaryOperator<Robot<?>>, ? extends DevoOutcome>, File> bestVideo(
    ...
    return Accumulator.Factory.<Event<?, ? extends UnaryOperator<Robot<?>>, ? extends DevoOutcome>>last().then(
        event -> {
          ...
          SortedMap<Long, String> terrainSequence = it.units.erallab.evolution.locomotion.Starter.getSequence((String) event.getAttributes().get("terrain"));
