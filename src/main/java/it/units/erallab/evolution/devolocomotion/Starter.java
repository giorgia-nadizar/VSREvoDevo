package it.units.erallab.evolution.devolocomotion;
...
import it.units.erallab.evolution.builder.devofunction.*;
import it.units.erallab.evolution.builder.evolver.EvolverBuilder;
import it.units.erallab.evolution.builder.evolver.PairsTree;
import it.units.erallab.evolution.builder.evolver.TreeAndDoubles;
...
import static it.units.erallab.evolution.devolocomotion.NamedFunctions.*;
...
public class Starter extends Worker {
  ...
  public final static Settings PHYSICS_SETTINGS = it.units.erallab.evolution.locomotion.Starter.PHYSICS_SETTINGS;
    ...
    if (!terrainName.contains("-rnd")) {
      ...
      if (distanceBasedDevelopment) {
        devoLocomotion = new DistanceBasedDevoLocomotion(
            ...
            it.units.erallab.evolution.locomotion.Starter.PHYSICS_SETTINGS
        );
      } else {
      ...
      return Misc.cached(devoLocomotion, it.units.erallab.evolution.locomotion.Starter.CACHE_SIZE);
    }
    return r -> {
      Task<UnaryOperator<Robot<?>>, DevoOutcome> devoLocomotion;
      if (distanceBasedDevelopment) {
  ...
  private static Evolver<?, UnaryOperator<Robot<?>>, DevoOutcome> buildEvolver(String evolverName, String devoFunctionName, UnaryOperator<Robot<?>> target, Function<DevoOutcome, Double> outcomeMeasure) {
    ...
    for (String piece : devoFunctionName.split(it.units.erallab.evolution.locomotion.Starter.MAPPER_PIPE_CHAR)) {
  ...
  public static EvolverBuilder<?> getEvolverBuilderFromName(String name) {
    ...
    return it.units.erallab.evolution.locomotion.Starter.getEvolverBuilderFromName(name);
  }
